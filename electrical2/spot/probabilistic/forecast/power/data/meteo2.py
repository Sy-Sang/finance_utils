...
meteo_tuple = namedtuple("meteo_tuple", ["title", "columns"])


class PFDInterFace:
    cma = meteo_tuple("cma", cma_new_energy_args)
    ecmwf = meteo_tuple("ecmwf", ecmwf_new_energy_args)
    ...
    @classmethod
    def norm_by_meteo_key(cls, matrix: numpy.ndarray, non_grid_norm: Type[Norm], grid_norm: Type[Norm], dims: list[str],
                          non_grid_dims: int = 7,
        ...
        for i, k in enumerate(meteo_keys):
            ...
            p_dic[k] = grid_norm.params(column, *args, **kwargs)
            grid_index += grid_len
        ...
        for i in it:
            if i < non_grid_dims:
                rm = new_or_stack(rm, non_grid_norm.f(matrix[:, i], *args, **kwargs))
            else:
                rm = new_or_stack(rm, grid_norm.f_with_params(matrix[:, i], p_dic[meteo_keys[k_index]]))
                c_counter += 1
    ...
    @classmethod
    def time_periodic_encoding(cls, matrix: numpy.ndarray, time_column_index: int = 0):
        timeline = [TimeStamp(i) for i in matrix[:, time_column_index]]
        timefeature_list = [[
            i.timestamp(),
        ...
        return timefeature_array, timefeature_array.shape[1]
